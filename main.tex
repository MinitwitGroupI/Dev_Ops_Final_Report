...
We audited our partner groups website and created issues for known issues that were discovered \footnote{\url{https://github.com/organizationGB/DevOps/issues/23}} \footnote{\url{https://github.com/organizationGB/DevOps/issues/25}}. 
...
SonarQube and CodeClimate were added and integrated with our repository, but ultimately ignored, because the warnings given were mostly irrelevant. Likewise, we had set up linting but it was ignored as it was implemented late in the development cycle. We should've created a pre-commit hook that didn't allow pushing before all linting passes. 
...
Optimally we should have had a test environment as well as a production environment. Some changes were unpredictable when tested locally, and when pushed into production, sometimes caused the application to go down. Ensuring a test environment would have helped to mitigate this issue.
